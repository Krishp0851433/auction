...
abstract class Model
{
    ...
    public static function find($cond = null, string $groupBy = null, string $orderBy = null,
                                $limit = null): array
    {
    ...
    private function getColumnNames(): array
    {
        ...
        try {
            $reflct = new ReflectionClass(get_called_class());
            $reflectProps = $reflct->getProperties();
            $props = array_column(array_map(function ($a) {
                $a->setAccessible(true);
                return [$a->getName(), $a->getValue($this)];
            }, $reflectProps), 1, 0);
        } catch (ReflectionException $e) {
            Logger::getLogger()->crtical("Reflection error: ", ['exception' => $e]);
            die();
        }
